document.addEventListener("DOMContentLoaded", () => {
  ...
  // Elements for displaying logged-in user info
  ...
  const signupLink = document.querySelector(
    ".login-form-container form p:last-of-type a"
  );
  ...
  // Hàm render thông tin người dùng nếu đã đăng nhập (đọc từ localStorage.currentUser)
  function renderUserInfo() {
    ...
    if (currentUser) {
      if (userInfoContainer && userNameSpan) {
        ...
        userNameSpan.textContent = currentUser.name || currentUser.email || 'Người dùng';
      }
      if (homeUserInfo && homeUserName) {
        homeUserInfo.style.display = '';
        homeUserName.textContent = currentUser.name || currentUser.email || 'Người dùng';
      }
      // Ẩn nút đăng nhập khi đã đăng nhập
      if (loginBtn) loginBtn.style.display = 'none';
    } else {
      if (userInfoContainer) userInfoContainer.style.display = 'none';
      if (homeUserInfo) homeUserInfo.style.display = 'none';
  ...
  const originalLoginFormHTML = loginFormElement
  ...
  let vehicleDetailsContainer = document.querySelector(
    "#vehicle-details-container"
  );
  let closeVehicleDetails = document.querySelector("#close-vehicle-details");
  let vehiclesSliders = document.querySelectorAll(
    ".vehicles-slider .box a.btn, .featured-slider .box a.btn"
  );

  // Lấy tất cả các nút "Xem thêm" trong phần Dịch vụ
  const serviceButtons = document.querySelectorAll(
    ".services .box-container .box a.btn"
  );

  // --- THÊM: Lấy nút Yêu cầu Lái thử ---
  const requestTestDriveBtn = document.querySelector(
    "#vehicle-details-container .info .btn"
  );
  ...
  function showLoginForm() {
    if (loginFormElement) {
      // Xóa event listener submit cũ (cả signup handler nếu trước đó đã gán)
      try {
        loginFormElement.removeEventListener('submit', handleSignup);
      } catch (err) {}
      try {
        if (loginFormElement.originalSubmitHandler)
          loginFormElement.removeEventListener('submit', loginFormElement.originalSubmitHandler);
      } catch (err) {}

      // Khôi phục HTML gốc và reset form
      ...
      const newLoginPassword =
        loginFormElement.querySelector("#login-password");
      const newLoginErrorMessage = loginFormElement.querySelector(
        "#login-error-message"
      );
      ...
      // Re-attach original login submit handler (nếu có)
      if (loginFormElement.originalSubmitHandler) {
        loginFormElement.addEventListener('submit', loginFormElement.originalSubmitHandler);
      }
    }
  }
  ...
  function showSignupForm(e) {
    ...
    if (loginFormElement) {
      loginFormElement.removeEventListener(
        "submit",
        loginFormElement.originalSubmitHandler
      ); // Giả định loại bỏ handler cũ
  ...
  if (loginBtn && loginForm) {
    loginBtn.addEventListener("click", () => {
      ...
      const currentErrorMsg = loginForm.querySelector(
  ...
  if (loginFormElement) {
    ...
    loginFormElement.originalSubmitHandler = function(e) {
      ...
      const errorMsgDiv = loginFormElement.querySelector(
        "#login-error-message"
      );
      ...
      // Kiểm tra đăng nhập với dữ liệu từ localStorage (kiểm tra tách: email và password)
      const users = JSON.parse(localStorage.getItem('users') || '[]');
      const userByEmail = users.find(u => u.email === emailValue);
      if (!userByEmail) {
        errorMessage = "Email chưa được đăng ký.";
      } else if (userByEmail.password !== passwordValue) {
        errorMessage = "Mật khẩu không chính xác.";
      } else {
        // assign user for later success flow
    ...
    // Gán handler cho form Login
    loginFormElement.addEventListener("submit", loginFormElement.originalSubmitHandler);
  }
  // --- HẾT LOGIC XỬ LÝ FORM ĐĂNG NHẬP ---

  // Hiển thị thông tin người dùng nếu đã đăng nhập khi load trang
  renderUserInfo();

  // Lấy form bên trong sell-car-form-container để thay đổi nội dung
  const sellCarFormContent = sellCarForm
    ? sellCarForm.querySelector("form")
    : null;

  // --- LOGIC TÁI SỬ DỤNG SELL CAR FORM CHO CÁC DỊCH VỤ KHÁC (BỔ SUNG) ---
  const originalSellCarFormHTML = sellCarFormContent
    ...
    serviceButtons.forEach((button) => {
      button.addEventListener("click", (e) => {
        ...
        // LOGIC ĐẶC BIỆT CHO ĐẶT HÀNG XE (Giả sử bạn đã đặt tên là "Đặt hàng xe")
          ...
          sellCarFormContent.addEventListener(
            "submit",
            function handleServiceSubmit(submitEvent) {
              submitEvent.preventDefault();
              alert(
                `Yêu cầu đặt hàng xe đã được gửi. Chúng tôi sẽ liên hệ với bạn sớm nhất.`
              );
              sellCarForm.classList.remove("active");
              // Xóa listener sau khi xử lý để tránh ảnh hưởng đến các lần mở form khác
              sellCarFormContent.removeEventListener(
                "submit",
                handleServiceSubmit
              );
            }
        ...
        // LOGIC CHUNG CHO TẤT CẢ CÁC DỊCH VỤ CÒN LẠI
          ...
          sellCarFormContent.addEventListener(
            ...
            function handleServiceSubmit(submitEvent) {
              submitEvent.preventDefault();
              alert(
                `Yêu cầu tư vấn dịch vụ "${serviceTitle}" đã được gửi. Chúng tôi sẽ liên hệ với bạn sớm nhất.`
              );
              sellCarForm.classList.remove("active");
              // Xóa listener sau khi xử lý để tránh ảnh hưởng đến các lần mở form khác
              sellCarFormContent.removeEventListener(
    ...
    vehiclesSliders.forEach((button) => {
      button.addEventListener("click", (e) => {
        ...
        if (box) {
          ...
          // Lấy thông tin cơ bản; nếu nút chứa data-* thì ưu tiên dùng data đó
          const clickedLink =
            e.target.closest("a.view-details") || e.target.closest("a.btn");
          if (
            clickedLink &&
            clickedLink.dataset &&
            (clickedLink.dataset.image ||
              clickedLink.dataset.name ||
              clickedLink.dataset.price)
          ) {
            imgSrc =
              clickedLink.dataset.image ||
            ...
            name =
              ...
              "";
            price = clickedLink.dataset.price || price;
          } else {
          ...
          if (box.closest(".vehicles-slider")) {
            ...
            const baseSpecs = pElement
              ? Array.from(pElement.childNodes)
                  .filter(
  ...
  // Khi mở modal chi tiết xe, cập nhật nút Thêm vào giỏ hàng trong modal
  (function setupModalAddToCart() {
    const modalBtn = document.querySelector("#modal-add-to-cart");
    const detailsContainer = document.getElementById(
      "vehicle-details-container"
    );
    if (!modalBtn || !detailsContainer) return;
    // Delegate click to document because modal content is dynamic
    detailsContainer.addEventListener("click", (e) => {
      if (e.target && e.target.id === "modal-add-to-cart") {
        e.preventDefault();
        // read data attributes set previously when modal opened
        const name =
          modalBtn.dataset.name ||
          document.getElementById("detail-vehicle-name").textContent.trim();
        const priceRaw =
          modalBtn.dataset.price ||
          document
            .getElementById("detail-vehicle-price")
            .textContent.replace(/[^0-9\.]/g, "") ||
          "0";
        const price = Number(priceRaw.replace(/,/g, "")) || 0;
        const image =
          modalBtn.dataset.image ||
          document.getElementById("detail-vehicle-img").getAttribute("src") ||
          "";
        if (typeof window.__BTL_ADD_TO_CART === "function") {
          window.__BTL_ADD_TO_CART({ name, price, image, qty: 1 });
          alert(`Đã thêm ${name} vào giỏ hàng`);
        } else {
          alert("Chức năng giỏ hàng chưa khởi tạo");
        }
      }
    });

    // Also update modal button data when modal is shown via vehicleDetailsContainer being activated
    const observer = new MutationObserver(() => {
      if (!detailsContainer.classList.contains("active")) return;
      // set modal btn data from modal DOM
      const img =
        document.getElementById("detail-vehicle-img")?.getAttribute("src") ||
        "";
      const name =
        ...
        "";
      let priceText =
        document.getElementById("detail-vehicle-price")?.textContent || "";
      const priceNum = Number(priceText.replace(/[^0-9\.]/g, "")) || 0;
      modalBtn.dataset.name = name;
      modalBtn.dataset.price = priceNum;
      modalBtn.dataset.image = img;
    });
  ...
  // Ensure scroll-padding-top matches header height so in-page anchors and top headings are not hidden
  function updateScrollPadding() {
    const headerEl = document.querySelector(".header");
    if (!headerEl) return;
    const rect = headerEl.getBoundingClientRect();
    const height = Math.ceil(rect.height);
    // set CSS scroll-padding-top dynamically
  ...
  // Run on load and when window resizes; also run periodically after header state changes
  updateScrollPadding();
  window.addEventListener("resize", updateScrollPadding);

  // Also update after small delay on scroll to account for header.active size change
  let scrollPaddingTimeout;
  window.addEventListener("scroll", () => {
    clearTimeout(scrollPaddingTimeout);
    scrollPaddingTimeout = setTimeout(updateScrollPadding, 80);
  });
  ...
  // --- Review form: star rating + submit handler ---
  (function setupReviewForm() {
    ...
    if (stars.length) {
      stars.forEach((star) => {
        star.addEventListener("mouseenter", () => {
          ...
          stars.forEach((s) =>
            s.classList.toggle("hovered", parseInt(s.dataset.rating) <= r)
          );
        });
        ...
        star.addEventListener("click", () => {
          ...
          stars.forEach((s) =>
            s.classList.toggle(
              "selected",
              parseInt(s.dataset.rating) <= currentRating
            )
          );
        });
      });
    ...
    reviewForm.addEventListener("submit", (e) => {
      ...
      const name = reviewForm
        .querySelector('input[name="reviewer"]')
        .value.trim();
      ...
      // Create new slide HTML and append to Swiper wrapper
      const wrapper = document.querySelector(".reviews-slider .swiper-wrapper");
      if (wrapper) {
        const slide = document.createElement("div");
        slide.className = "swiper-slide box";
        let starsHTML = "";
        for (let i = 1; i <= 5; i++)
          starsHTML += `<i class="fas fa-star" style="color: var(--yellow);"></i>`;
        slide.innerHTML = `
                  <img src="image/pic-1.png" alt="Client">
        ...
        // If Swiper instance exists, update it (Swiper auto-init may vary), try to update pagination
        if (typeof Swiper !== "undefined") {
          // Try to re-init or update - simplest: trigger a reload by toggling loop off/on not done here.
          // Many sites call mySwiper.update(); but we don't have mySwiper reference for reviews slider.
        }
      }
